...
# 반경 방향 편차도 라벨 자신에서는 잴 수 없다. 0.15 m = 3셀이고 주 지표 `f1@10cm`의 허용
# 오차(2셀)보다 한 셀 넓다.
#
# **[2026-09-01] 0.20 → 0.15로 바꿨다.** 옛 값 0.20(= 허용 오차의 2배)은 스윕 없이 정한
# 값이었다. 좁히면 무벌점 구간이 4셀 → 3셀이 되어 **더 많은 광선이 gradient를 받는다.**
# 스윕은 `configs/sweep_range_dr_beta.sh` / `runs/range_dr_beta`.
DEFAULT_DELTA_R_M = 0.15

# Huber의 전환점 [m]. `e^eff <= beta`는 이차, 그 위는 선형이다. **미터 단위로 둔다** --
# 정규화된 스케일에서 주면 실효 오차가 항상 beta보다 훨씬 작아져 Huber가 순수 L2로 퇴화하고
# "outlier에 덜 끌려간다"는 도입 이유가 사라진다.
#
# **[2026-09-01] 0.10 → 0.15로 바꿨다.** 옛 값 0.10은 dead zone 통과 후의 전형적 잔차
# 규모(`range_mae` 실측 0.235 m − δ_R 0.20 m)에서 왔는데, δ_R을 0.15로 좁히면 그 잔차가
# 0.085 m로 커진다. 키우면 작은 잔차가 이차 구간에 들어가 gradient가 `e/beta`로 줄어든다
# -- δ_R을 좁힌 것과 방향이 반대라 두 변화가 서로 상쇄하는 쪽이다. 같은 스윕에서 봤다.
DEFAULT_HUBER_BETA_M = 0.15
