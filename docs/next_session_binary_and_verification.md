...
이 문서는 **다음 세션이 먼저 읽는 문서**다. 지금 어디까지 됐고, 무엇이 확정이고, 다음에
무엇을 결정해야 하는지가 여기 있다. 근거 정본은
[`finetune_overfitting_diagnosis.md`](finetune_overfitting_diagnosis.md) **§15–§19**.
...
## 0. 지금 논의 중인 것 -- 사용자가 예측 영상을 보고 "예상치 못한 결과"라고 했다

전체 시퀀스 추론 영상을 만들어 사용자가 직접 확인하는 단계다. **`raws1` 영상을 본 뒤
"예상치 못한 결과가 나와서 논의하자"고 했고, 아직 그 내용은 듣지 못했다.** 검증 실험은
사용자 요청으로 **일시 중단** 상태다.
