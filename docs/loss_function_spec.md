...
| $\delta_R^{-}$ | **0.15 m** (3셀) | `DELTA_R_M` | `f1@10cm` 허용오차(2셀)보다 한 셀 넓다. **[2026-09-01] 0.20 → 0.15** — 옛 값은 스윕 없이 정한 값이었다(`configs/sweep_range_dr_beta.sh`). **$\delta > \delta_R$이어도 무방하다** — 아래 주석 |
| $\delta_R^{+}$ | **0.15 m** (대칭) | `DELTA_R_OVER_M` | **미결** — §8.6 |
| $\beta$ | **0.15 m** | `HUBER_BETA_M` | §8.5. **[2026-09-01] 0.10 → 0.15** — $\delta_R$을 좁힌 것과 짝이고 방향은 반대다(작은 잔차를 이차 구간에 넣는다) |
