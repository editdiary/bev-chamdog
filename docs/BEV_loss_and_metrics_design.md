...
### 2.9 주 판정 지표 변경 (2026-08-23) — `iou_free`는 남고 `f1@10cm`은 강등된다

**이 절은 지표의 *역할* 변경 기록이다.** 정의·계산은 하나도 바꾸지 않았고 전부 그대로
보고된다. 바뀐 것은 **무엇으로 채택/기각을 판정하는가**다. 근거 정본은
[`soft_boundary_loss_design.md`](soft_boundary_loss_design.md) **§13.6.1(σ_run 실측)**,
**§13.7(전 지표 재판독)**, **§14(프로토콜)**.

**왜.** 같은 config·같은 시드로 5런을 돌려 런 간 표준편차(σ_run)를 처음 실측했다.
`grid_sample` backward의 atomicAdd 때문에 같은 시드로도 런이 갈린다.

| 지표 | σ_run | 상대σ |
|---|---|---|
| `iou_free` | **0.0007** | 0.09 % |
| `fatal_rate` / `free_miss_rate` | **0.0005** | 0.4 ~ 0.5 % |
| `missed_obstacle_rate` | 0.0006 | 1.6 % |
| `f1@40cm` | 0.0021 | 0.23 % |
| `range_mae` | 0.0024 | 1.0 % |
| **`f1@10cm`** | **0.0037** | 0.69 % |
| `f1@20cm` | 0.0040 | 0.51 % |

**`f1@10cm`의 σ_run이 가장 크다.** 그리고 이 프로젝트가 재려는 효과 크기가 0.002~0.005라
**단일 런으로는 원리적으로 해상되지 않는다.** 실제로 옛 스윕이 노이즈를 결론으로 읽었다
(설계 문서 §10.3의 "α 단조성"이 기각됐다).

게다가 `f1@10cm`은 2셀 안을 요구하는 **가장 까다로운 지표**이고 그 천장을 라벨 정밀도가
잡고 있다([`experiment_history.md`](experiment_history.md) §4.7). 즉 **여기서 개선이 안
나오는 것은 실패가 아니라 예상된 결과**다.

**변경.**

| 역할 | 지표 |
|---|---|
| 체크포인트 선택 | `iou_free` (**변경 없음**) |
| **주 판정 -- 안전** | `fatal_rate`, `missed_obstacle_rate` |
| 주 판정과 짝 | `free_miss_rate` (**반드시 같이** 읽는다 -- 교환 관계다) |
| 주 판정 -- 안정성 | 되올림, train/val KL 배율 (설계 문서 §13.6.1) |
| 제약(악화 없음 확인) | `iou_free` + ring 3개 |
| 참고 | `f1@40cm`, `range_mae` |
| **강등** | **`f1@10cm`, `f1@20cm`** -- 보고는 계속, 단독 판정 금지 |

**`range_abs_p90`은 판정에 쓰지 않는다** -- 광선 표본 간격에 양자화되어 전 런에서 정확히
0.5500이 나온다(변별력 0).

이 변경으로 실제 결론이 바뀐 예: soft-boundary loss 대 역빈도 가중 CE에서 `f1`·`iou`는
전부 노이즈 안인데 **`fatal_rate` −5.2 %(14σ), `missed_obstacle` −11.5 %(8.4σ),
되올림 −74 %(94σ)**였다(설계 문서 §13.7.2). 옛 프로토콜은 이 loss의 실제 성과를 놓치고
노이즈(`f1` +0.0103)를 성과로 보고하고 있었다.
